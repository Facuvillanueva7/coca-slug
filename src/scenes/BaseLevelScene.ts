...
export abstract class BaseLevelScene extends Phaser.Scene {
  ...
  createBase(characterKey: string): void {
    ...
    this.physics.add.overlap(this.player, this.enemies, (_p, e) => this.player.takeDamage((e as EnemyBase).typeId.includes('llama') ? 12 : 8));
  ...
  protected commonUpdate(time: number): void {
    ...
    this.enemies.children.each((child) => (child as EnemyBase).behave(this.player.x, time));
